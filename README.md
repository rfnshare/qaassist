...
Step 0016 board knowledge and requirement upload foundation is implemented for review. The repository contains a Chrome Manifest V3 extension under `apps/extension`, a Fastify TypeScript backend under `apps/api`, and shared TypeScript contracts under `packages/shared`.
...
The Azure DevOps PAT remains a local development backend-only fallback and is never requested by the extension. The product direction is seamless Azure/Microsoft auth later through Microsoft Entra/OAuth. The current board briefing and Story requirement analysis are deterministic preview logic, not LLM calls. Board knowledge is metadata-only; file content upload, parsing, indexing, and LLM ingestion are intentionally not implemented yet. Final test case generation, Azure Test Plans write-back, OAuth, database/storage, comments, bug creation, and automation generation are also intentionally not implemented yet.
...
13. In `Settings`, add a Board Knowledge metadata-only source and confirm no file content is read or uploaded.
14. In `Today`, click `Fetch board condition`.
15. Click `Generate QA briefing` after the board condition fetch succeeds.
16. Confirm real counts, story details, analysis, board knowledge metadata, and briefing claims only appear after successful backend calls. If backend PAT is missing, the UI should show `Azure DevOps backend token is not configured. Add it to local .env on the API server.`
