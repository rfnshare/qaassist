...
Step 0029 team reporting/analytics is implemented for review. The repository contains a Chrome Manifest V3 extension under `apps/extension`, a Fastify TypeScript backend under `apps/api`, and shared TypeScript contracts under `packages/shared`.
...
The Azure DevOps PAT remains a local development backend-only fallback and is never requested by the extension. The product direction is seamless Azure/Microsoft auth later through Microsoft Entra/OAuth. Deterministic board briefing, Story requirement analysis, and test case draft generation remain the default baseline. Board knowledge supports metadata plus a backend JSON-only extraction preview for manually pasted `.txt` and `.md` text. Story analysis can include only user-selected board knowledge metadata, capped extraction preview evidence, or a short user-confirmed note. Test case drafts are generated from available evidence only and can be reviewed, edited, approved for later export, rejected, or blocked in the Story panel. Step 0021 adds a readiness preview that shows which validated reviewed cases could become Azure Test Plans candidates, which cases remain blocked, and whether non-secret Test Plan/Suite target IDs are missing. Step 0022 adds the first explicit creation path: QA must select candidates manually and confirm before the backend creates test cases in Azure Test Plans. Step 0023 adds optional backend-only LLM provider status and a separate Story AI assist route; no API key is stored in the extension, AI output is suggestion-only, and deterministic analysis remains visible. Step 0024 maps validated reviewed cases into planning-only automation candidates with readiness, reasons, blockers, and suggested surfaces. Step 0025 packages selected current-session output as JSON or Markdown for copy/download only. Step 0026 adds preview-only helpers for bug drafts, comments, state transitions, and attachment metadata. Step 0027 adds optional backend-only AI refinement for draft cases, automation mapping, and write-back helper wording. Step 0028 adds a deterministic automation scaffold preview for selected automation candidates, showing likely files, dependencies, and gaps without creating code. Step 0029 adds read-only team analytics that summarize current QA Assist session inputs such as board preview, reviews, readiness, creation outcomes, automation mapping/scaffolds, exports, and write-back previews. These refinements, scaffolds, and analytics are separate planning/reporting layers and are never auto-applied. They do not create bugs, post comments, transition states, upload attachments, create automation files, update repositories, call Azure DevOps, or invent historical reporting. Blocked and non-selected cases are not created. Board knowledge is not automatically indexed or linked. OAuth, database/storage, live comments, live bug creation, story linking, final persisted approved test cases, automation generation, historical analytics, and AI auto-application are intentionally not implemented yet.
...
17. Select automation candidates manually and click `Preview automation scaffold`, then confirm likely files/dependencies/gaps render without creating files.
18. Generate team analytics from Today after board or Story session data exists, and confirm it says read-only analytics with no external system updates.
19. Request AI refinements for draft cases or automation mapping only after the deterministic output exists, and confirm suggestions render separately without changing the baseline data.
20. Generate an export package and confirm it is copy/download only with no external write button.
21. Preview a write-back helper and confirm the copy says nothing was written to Azure DevOps.
22. Request AI wording refinement for the helper preview only when the backend provider is available, and confirm no submit action appears.
23. Confirm description, acceptance criteria, and linked extraction previews display as safe text previews, not executable HTML.
24. In `Settings`, test `System`, `Light`, and `Dark` theme modes.
25. In `Settings`, enter an Azure DevOps Services or TFS URL and click `Connect`.
26. Select a discovered project and team board, or use advanced local preview only when discovery cannot run in local development.
27. In `Settings`, add a Board Knowledge metadata-only source and confirm the file selector captures metadata only.
28. Paste `.txt` or `.md` text into Extraction preview and confirm the backend returns status, evidence, warnings, and preview text without storing or indexing it.
29. In `Today`, click `Fetch board condition`.
30. Click `Generate QA briefing` after the board condition fetch succeeds.
31. Confirm real counts, story details, analysis, board knowledge metadata, extraction preview, draft review decisions, Test Plans readiness/creation, automation mapping, scaffold previews, analytics, export packages, write-back helper previews, AI refinement suggestions, and briefing claims only appear after successful backend calls. If backend PAT is missing, the UI should show `Azure DevOps backend token is not configured. Add it to local .env on the API server.`
