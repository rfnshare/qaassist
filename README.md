...
Step 0015 deterministic requirement and gap analysis for the Story workspace is implemented for review. The repository contains a Chrome Manifest V3 extension under `apps/extension`, a Fastify TypeScript backend under `apps/api`, and shared TypeScript contracts under `packages/shared`.
...
The Azure DevOps PAT remains a local development backend-only fallback and is never requested by the extension. The product direction is seamless Azure/Microsoft auth later through Microsoft Entra/OAuth. The current board briefing and Story requirement analysis are deterministic preview logic, not LLM calls. Final test case generation, Azure Test Plans write-back, OAuth, LLM provider calls, file upload, database/storage, comments, bug creation, and automation generation are intentionally not implemented yet.
...
7. Click `Analyze requirements` and confirm the output is labeled `Evidence-bound preview`.
8. Confirm gaps/questions and likely test areas are clearly non-final and need confirmation.
9. Confirm description and acceptance criteria display as safe text previews, not executable HTML.
10. In `Settings`, test `System`, `Light`, and `Dark` theme modes.
...
13. In `Today`, click `Fetch board condition`.
14. Click `Generate QA briefing` after the board condition fetch succeeds.
15. Confirm real counts, story details, analysis, and briefing claims only appear after successful backend calls. If backend PAT is missing, the UI should show `Azure DevOps backend token is not configured. Add it to local .env on the API server.`
